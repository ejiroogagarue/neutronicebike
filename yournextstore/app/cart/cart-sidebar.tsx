			...
			<SheetContent className="flex flex-col w-full sm:max-w-lg">
				...
				{items.length === 0 ? (
					<div className="flex-1 flex flex-col items-center justify-center gap-4 py-12">
						...
						<Button variant="outline" onClick={closeCart} className="min-h-[44px]">
							Continue Shopping
						</Button>
					</div>
				) : (
						...
						<SheetFooter className="border-t border-border pt-4 mt-auto">
							<div className="w-full space-y-4">
								...
								<button
									type="button"
									onClick={closeCart}
									className="w-full min-h-[44px] text-sm text-muted-foreground hover:text-foreground transition-colors flex items-center justify-center"
								>
									Continue Shopping
								</button>
							</div>
